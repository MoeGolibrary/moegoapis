...
| `company_id`                    | string        | Yes      | Company ID, used for permission control                                    |
| `business_ids`                  | Array(string) | No       | List of business unit IDs to include in the report (if null, includes all) |
| `condition.id`                  | string        | Yes      | Report definition ID                                                       |
| `condition.query_period`        | Interval      | Yes      | Time period for data collection                                            |
| `condition.group_by_field_keys` | Array(string) | No       | Optional list of field keys to group data by                               |
